...
- **Different AI agent entirely, per role.** Beyond swapping models within Claude, an individual Lead/worker (or Atlas himself) can run on **xAI's Grok CLI**, **OpenAI's Codex CLI**, **Google's Antigravity CLI** (`agy`), or **Cursor's CLI** (`cursor-agent`) instead. Each wraps that provider's own agentic CLI (tool belt, sandboxing, and permission handling included), so it is a genuine second agent, not just a different model string. This is an advanced option: switch a chat with `/model grok-cli`, `/model codex-cli`, `/model agy-cli`, or `/model cursor-cli`, or pick it from the "AI backend" selector next to Provider/Model in Settings/Workers (for Antigravity and Cursor the Model field stays visible with a Fetch button listing what that CLI accepts: Antigravity's Gemini/Claude/GPT-OSS labels, or every model id your Cursor account can run, and empty means the default). Claude stays the default everywhere unless you opt in. The **Antigravity** backend is wired in deepest: it gets your MyAgens tools (memory, kanban, crew, skills, `send_file`, connectors), your persona and `work.md` playbook, and live tool status with real Approve/Deny buttons, so it behaves like Atlas rather than a stock Gemini agent.
