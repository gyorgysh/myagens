...
- **Different AI agent entirely, per role.** Beyond swapping models within Claude, an individual Lead/worker (or Atlas himself) can run on **xAI's Grok CLI**, **OpenAI's Codex CLI**, **Google's Antigravity CLI** (`agy`), **Cursor's CLI** (`cursor-agent`), or **OpenCode's CLI** (`opencode`) instead. Each wraps that provider's own agentic CLI (tool belt, sandboxing, and permission handling included), so it is a genuine second agent, not just a different model string. This is an advanced option: switch a chat with `/model grok-cli`, `/model codex-cli`, `/model agy-cli`, `/model cursor-cli`, or `/model opencode-cli`, or pick it from the "AI backend" selector next to Provider/Model in Settings/Workers (for Antigravity, Cursor, Codex and OpenCode the Model field stays visible, with a Fetch button where the CLI can list models: Antigravity's Gemini/Claude/GPT-OSS labels, every model id your Cursor account can run, or OpenCode's `provider/model` ids, and empty means the default). Claude stays the default everywhere unless you opt in. The **Antigravity**, **Codex**, **Cursor** and **OpenCode** backends are wired in deepest: they get your MyAgens tools (memory, kanban, crew, skills, `send_file`, connectors), your persona and `work.md` playbook, and live tool status with real Approve/Deny buttons, so they behave like Atlas rather than a stock agent. Codex runs against a MyAgens-owned Codex configuration, so your own `~/.codex` setup and sessions are untouched (and your `config.toml` model does not carry over, hence the Model field). For usage-based OpenAI Platform billing instead of a ChatGPT subscription, set an **OpenAI API key** in Settings when Codex is selected (or `CODEX_API_KEY` in `.env`). OpenCode injects MCP and a gate plugin via env (`OPENCODE_CONFIG_CONTENT` / `OPENCODE_CONFIG_DIR`) and never writes your `~/.config/opencode` or project `opencode.json`. Cursor is the one that has to reach into your project: it reads its hook and MCP configuration only from the folder you are working in, so MyAgens writes into `<project>/.cursor/` for the duration of a turn, merging with whatever is already there and restoring it afterwards (including after a crash), with a per-agent checkbox to switch that off and run the plain CLI.
- **Run entirely on a local model.** A fourth backend, **Ollama (local chat)**, talks straight to a local Ollama server instead of the Claude CLI, so a small local model (14-24B) that could never prefill the CLI's large system prompt stays fast and fully Anthropic-independent. It hands the model a tiny hand-built prompt plus a single `Bash` tool (still gated by the normal approval flow) and keeps its own per-session history. Pick it from the same "AI backend" selector and set the agent's model to an installed Ollama model name (`ollama list`); tune it with the `OLLAMA_*` env vars. Trim the system prompt per agent with the **System prompt** checkboxes (drop `work.md`, persona, known paths, or memory recall) so a small local model doesn't pay for a big playbook on every turn.
- **Per-agent fallback.** Give any agent a primary model plus a fallback backend and model of its own. Two triggers switch to it: *proactively*, when the cached usage probe shows the plan is at or over the rate-limit threshold, background turns move to the fallback; and *reactively*, when a turn hits a usage or rate-limit error mid-flight (interactive turns included), it retries once on the fallback and tells you in chat. The fallback can be a different backend entirely (Ollama, Grok, Codex, Antigravity, Cursor, or OpenCode), and every Lead and worker fails over independently of Atlas.
...
| `OLLAMA_NUM_CTX` | no | Context window for the `ollama` backend (default `8192`; clamped so a large custom Modelfile can't swap-thrash the host) |
| `OLLAMA_KEEP_ALIVE` | no | How long Ollama keeps the model resident between turns (default `30m`) |
| `APPROVAL_TIMEOUT_MS` | no | Approval wait before auto-deny (default `300000`) |
| `TURN_STALL_TIMEOUT_MS` | no | Abort a turn after this long (ms) of complete backend silence so a hung CLI can't leave a chat stuck "busy"; extended by the task-run timeout while a tool call is in flight (default `1800000`, 30 min; `0` disables) |
...
| Content and config | `GET\|PUT /api/prompt`, `POST /api/prompt/restore`, `GET /api/claude-files`, `GET\|PUT /api/claude-files/content`, `GET /api/languages`, `GET /api/connectors`, `PUT /api/connectors/:id`, `POST /api/connectors/:id/accounts`, `PUT\|DELETE /api/connectors/:id/accounts/:accountId`, `GET\|PUT /api/branding`, `GET /api/conversations/search` |
...
- **Tmux mode (persistent instances)**: a per-agent toggle (Atlas and every worker/Lead) that hosts the agent's conversation in one long-lived Claude Code TUI inside a named tmux session, instead of a fresh headless run per message. Replies are captured by tailing the CLI's own session transcript — streamed live into Telegram/panel chat with tool-call events and real token usage, nothing injected into the conversation. The conversation survives bot restarts (re-adopted on boot; respawns resume the same session), `/new` resets it, and `/status` shows where it lives. Watch or take over with `tmux attach -t myagens-<agent>` or from the panel's live viewer — embedded in Settings, one click away via the Terminal button on each tmux-mode agent's card in the Agents view, expandable into a large centered cinema view — read-only until the explicit "Take control" switch; other agents can observe/steer it via `tmux_*` MCP tools. Requires Full autonomy and tmux installed (the installer now sets it up; the toggle greys out with a hint when it's missing); agents that stay opted out keep the default per-turn SDK path.
...
- **Per-agent model fallback**: give Atlas, any Lead, or any worker a fallback backend/provider/model of its own, and it fails over independently of everyone else. Two triggers switch to it: proactively, when the cached usage probe shows the plan is at or over the rate-limit threshold, background turns switch (interactive turns aren't redirected proactively); and reactively, when any turn hits a usage or rate-limit error mid-flight — interactive turns included — the same turn is retried once on the fallback and the chat is notified. The fallback target can be a whole different backend (Ollama, Grok CLI, or Codex CLI), not just another Claude provider; a cross-backend failover starts a fresh conversation. Configured via `PUT /api/agent` (Atlas) or each worker's settings.
...
- **Turn stall watchdog**: a hung agent backend used to leave a chat stuck "busy" forever with no way to start another turn. A watchdog wired into every backend now aborts a turn after `TURN_STALL_TIMEOUT_MS` (default 30 min; `0` disables) of complete silence — no streamed text, tool calls, or approvals — extending the allowance by the task-run timeout while a tool call is genuinely in flight, and returns a friendly error. The heartbeat also raises a long-busy-chat signal nudging you to `/stop` a wedged chat.
- **Per-agent system-prompt slimming**: a "System prompt" control on Atlas and each worker/Lead drops sections from the prompt append — `work.md`, persona, known paths, or memory recall (excluding memory skips recall entirely) — trimming only our append, never the Claude Code preset. The point is smaller/local models, where a large `work.md` otherwise costs input tokens on every single turn.
...
- **In-panel updates**: the Updates view checks for a new version, applies it, and can roll back, mirroring `scripts/update.sh` without leaving the dashboard. After any update, restore, or `/reload` restart, the bot confirms over Telegram that it came back up ("Update successful: v0.6.5 → v0.6.6, back online") — closing the loop the new-version notice opened. It also renders a "What's new" section of releases newer than the installed version and a year-grouped Release history, parsed from the public `CHANGELOG.md` (falling back to the locally served copy at `GET /api/update/changelog` when GitHub is unreachable). The Feedback view shows a soft nudge linking here when the deployment is behind, and the Setup view surfaces the running version with a changelog link.
- **Connectors**: external-service integrations, each with a vault-backed credential slot and a per-connector read / write scope. Twenty-two are live with real MCP tool calls: **Notion** (search, read, create pages/databases), **Google Calendar** (list, create events), **Gmail** (list, read, send, draft, label, delete), **Google Drive** (list, read, create, update, move, share, delete), **Apple Calendar** (iCloud CalDAV: list, create, update, delete events), **Apple Mail** (iCloud IMAP/SMTP: list, read, search, send, delete), **Slack** (list channels, read history, post messages, reply in threads, search, upload files), **GitHub** (list repos and issues, read/write files, create issues and PRs, comment on issues), **Unreal Engine** (control a running UE 5.8+ editor via its built-in MCP plugin, no credential needed), **Unity** (control a running Unity Editor via the mcp-unity package), **Browser Sketchpad** (a local headless Playwright browser in its own persistent profile — agents open pages or local HTML files, click through flows, read console errors, and screenshot to verify their own web work, and can smoke-test live sites including logins when asked; no credential needed, agents confirm before entering credentials you didn't provide, and every action goes through the approval flow), **PostgreSQL** (list tables, describe schemas, read-only queries, and write-scoped mutating statements), **SQLite** (same, against a local database file), **Jira Cloud** (list projects, JQL search, read issue, apply transitions, create, comment), **Linear** (list teams/projects/states, search and read issues, create, move state, comment), **Bluesky** (search, timeline, notifications, post/reply, delete), **Mastodon** (any instance: search, home timeline, notifications, post/reply, delete), **Discord** (list channels, read messages, post as your bot), **Reddit** (search, browse subreddits, read posts with comments, submit, comment), **X** (post/reply, delete; the free API tier is write-only — reading needs a paid tier), **YouTube** (channel stats, list uploads, read comments, upload videos — private by default — update metadata, comment and reply), and **Facebook Pages** (Page info, feed with engagement counts, read comments, publish text/link/photo posts, comment as the Page, delete; personal profiles aren't supported by Meta's API). The seven social connectors are **multi-account**: add several named accounts (each with its own vault credential) and agents pick one by label per call, so one Lead can post as the company while another runs a side project. Read scope exposes only the read tools; write tools appear when you flip a connector to write scope. Connectors can also track an optional credential expiry date, shown as an ok/expiring/expired badge in the panel. Connector tools run through the normal approval flow in interactive mode and freely in autonomous (`full`) mode.
...
src/
  ...
  codex/
    ...
    codexHome.ts      private CODEX_HOME (auth symlink + our hooks.json) so the user's ~/.codex is untouched
    prompt.ts         codex runtime paragraph for the shared CLI system prompt
    toolMap.ts        codex tool names/args -> Bash/Write/Edit/... for status and approvals
  agy/
  ...
  opencode/
    runner.ts         AgentBackend wrapping OpenCode's CLI (json events, tool calls + usage)
    launch.ts         OPENCODE_CONFIG_CONTENT (MCP) + OPENCODE_CONFIG_DIR (plugin gate)
    toolMap.ts        OpenCode tool names -> canonical Bash/Read/Write/…
    prompt.ts         OpenCode's runtime paragraph for the shared CLI system prompt
  core/               surface-free layer shared by all agents and every front end
    backends.ts       AgentBackend registry (claude-agent-sdk default, grok-cli, codex-cli, agy-cli, cursor-cli, opencode-cli, ollama)
    ...
    connectors.ts     external-connector catalog (Notion, GCal, Gmail, Drive, Apple Cal/Mail, Slack, GitHub, Unreal, Unity, Browser Sketchpad, PostgreSQL, SQLite, Jira, Linear, Bluesky, Mastodon, Discord, Reddit, X, YouTube, Facebook Pages) + read/write scope + social multi-account CRUD
