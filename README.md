...
- **Remote Control**: a per-agent toggle (Atlas and every worker/Lead) that runs turns with Claude Code Remote Control, so a live session can be watched and steered from claude.ai/code or the Claude mobile app under the agent's own name. Off by default; Claude backend only, needs the installed `claude` CLI and a subscription sign-in.
