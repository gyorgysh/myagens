...
- **Different AI agent entirely, per role.** Beyond swapping models within Claude, an individual Lead/worker (or Atlas himself) can run on **xAI's Grok CLI**, **OpenAI's Codex CLI**, **Google's Antigravity CLI** (`agy`), or **Cursor's CLI** (`cursor-agent`) instead. Each wraps that provider's own agentic CLI (tool belt, sandboxing, and permission handling included), so it is a genuine second agent, not just a different model string. This is an advanced option: switch a chat with `/model grok-cli`, `/model codex-cli`, `/model agy-cli`, or `/model cursor-cli`, or pick it from the "AI backend" selector next to Provider/Model in Settings/Workers (for Antigravity, Cursor and Codex the Model field stays visible, with a Fetch button where the CLI can list models: Antigravity's Gemini/Claude/GPT-OSS labels, or every model id your Cursor account can run, and empty means the default). Claude stays the default everywhere unless you opt in. The **Antigravity**, **Codex** and **Cursor** backends are wired in deepest: they get your MyAgens tools (memory, kanban, crew, skills, `send_file`, connectors), your persona and `work.md` playbook, and live tool status with real Approve/Deny buttons, so they behave like Atlas rather than a stock Gemini, Codex or Cursor agent. Codex runs against a MyAgens-owned Codex configuration, so your own `~/.codex` setup and sessions are untouched (and your `config.toml` model does not carry over, hence the Model field). Cursor is the one that has to reach into your project: it reads its hook and MCP configuration only from the folder you are working in, so MyAgens writes into `<project>/.cursor/` for the duration of a turn, merging with whatever is already there and restoring it afterwards (including after a crash), with a per-agent checkbox to switch that off and run the plain CLI.
...
- **Per-agent fallback.** Give any agent a primary model plus a fallback backend and model of its own. Two triggers switch to it: *proactively*, when the cached usage probe shows the plan is at or over the rate-limit threshold, background turns move to the fallback; and *reactively*, when a turn hits a usage or rate-limit error mid-flight (interactive turns included), it retries once on the fallback and tells you in chat. The fallback can be a different backend entirely (Ollama, Grok, Codex, or Antigravity), and every Lead and worker fails over independently of Atlas.
