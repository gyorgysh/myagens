...
- **Per-agent fallback.** Give any agent a primary model plus a fallback backend and model of its own. Two triggers switch to it: *proactively*, when the cached usage probe shows the plan is at or over the rate-limit threshold, background turns move to the fallback; and *reactively*, when a turn hits a usage or rate-limit error mid-flight (interactive turns included), it retries once on the fallback and tells you in chat. The fallback can be a different backend entirely (Ollama, Grok, or Codex), and every Lead and worker fails over independently of Atlas.
