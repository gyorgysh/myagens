...
- **Different AI agent entirely, per role.** Beyond swapping models within Claude, an individual Lead/worker (or Atlas himself) can run on **xAI's Grok CLI**, **OpenAI's Codex CLI**, or **Google's Antigravity CLI** (`agy`) instead. Each wraps that provider's own agentic CLI (tool belt, sandboxing, and permission handling included), so it is a genuine second agent, not just a different model string. This is an advanced option: switch a chat with `/model grok-cli`, `/model codex-cli`, or `/model agy-cli`, or pick it from the "AI backend" selector next to Provider/Model in Settings/Workers (for Antigravity the Model field takes one of `agy models`' labels — Gemini, Claude, and GPT-OSS tiers — empty for its default). Claude stays the default everywhere unless you opt in.
...
src/
  ...
  core/               telegraf-free layer shared by all agents and the panel
    backends.ts       AgentBackend registry (claude-agent-sdk default, grok-cli, codex-cli, agy-cli)
