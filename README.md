...
- **Tmux mode (persistent instances)**: a per-agent toggle (Atlas and every worker/Lead) that hosts the agent's conversation in one long-lived Claude Code TUI inside a named tmux session, instead of a fresh headless run per message. The conversation survives bot restarts (re-adopted on boot), can be watched or taken over with `tmux attach -t myagens-<agent>`, from the panel's live viewer (read-only, with an explicit "Take control" switch), and other agents can observe/steer it via `tmux_*` MCP tools. Requires Full autonomy and tmux installed; agents that stay opted out keep the default per-turn SDK path.
